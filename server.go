...
func (server *Server) handle(c net.Conn, handler Handler) {
	...
	var ctx *Context = NewContext(c, newDataReader(c), newDataWriter(c)).InjectLogger(logger)

	server.Contexts = append(server.Contexts, ctx)
	handler.ServeTELNET(*ctx)
...
func (server *Server) RemoveContext(ctx *Context) {
	for i := len(server.Contexts) - 1; i >= 0; i-- {
		context := server.Contexts[i]
		if context == ctx {
			server.Contexts = append(server.Contexts[:i], server.Contexts[i+1:]...)
			return
		}
	}
}
